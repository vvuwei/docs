...
description: Learn how to set up a Celestia validator and full consensus node.
---

# Setting up Celestia consensus nodes
...
sed -i.bak -e "s/^persistent-peers *=.*/persistent-peers = \"$PERSISTENT_PEERS\"/" $HOME/.celestia-app/config/config.toml<br/>
</code></pre>
...
```sh
celestia-appd start --chain-id mocha-4
```
